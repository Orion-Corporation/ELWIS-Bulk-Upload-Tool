...
# Load Mcule supplier synonyms
with open("config/supplier_synonyms_mcule.json", "r") as f:
...
def process_sdf(file_path):    
    ...
    # supplier_name = "Enamine" # Set dynamically from Mcule SDF
    source = "Acquired"
    ...
    for mol in file:
        ...
        supplier_product_code = mol.GetProp("Product ID")
        ...
        if original_supplier_name not in supplier_synonyms:
            print(f"Error: Supplier name '{original_supplier_name}' not found in supplier_synonyms_mcule.json. Skipping molecule {supplier_product_code}")
            continue
        supplier_name = supplier_synonyms[original_supplier_name]
    
        # Set other molecule properties
        mol.SetProp("Supplier_Product_Code", mol.GetProp("Product ID"))
        mol.SetProp("Plate_ID", mol.GetProp("Multi container ID"))
        mol.SetProp("Well", mol.GetProp("Single container position"))
        mol.SetProp("Batch_Code", mol.GetProp("Ordered Mcule ID"))
        mol.SetProp("Amount", mol.GetProp("Amount (mg)") + " mg")
...
def main():
    ...
    sdf_file_path = 'MCULE/mcule_test.sdf'
